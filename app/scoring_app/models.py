...
class Frame(models.Model):
    ...
    roll_three = models.PositiveIntegerField(
        default=0, choices=list(zip(range(0, 11), range(0, 11)))
    )
    ...
    def frame_score(self):
        ...
        if self.is_strike and self.frame_no < 9:
            frame_one_ahead = (
                Frame.objects.filter(game_id=self.game_id, frame_no__gt=self.frame_no)
                .order_by("frame_no")
                .first()
            )
            frame_two_ahead = (
                Frame.objects.filter(
                    game_id=self.game_id, frame_no__gt=frame_one_ahead.frame_no
                )
                .order_by("frame_no")
                .first()
            )
